...
UNLUA_API int32 Global_LoadContext(lua_State *L)
{
	...
	if (GCodeContext.Path.Len() < 1 && GCodeContext.SourceCode.Len() < 1 && GCodeContext.ByteCode.Num() < 1)
	{
		if (FUnLuaDelegates::LoadLuaContext.IsBound())
		{
			LoadWithContext = FUnLuaDelegates::LoadLuaContext.Execute(ModuleName, CodeContext);
			if (LoadWithContext)
			{
...
UNLUA_API int32 Global_LoadContextAsset(lua_State *L)
{
	...
	if (FUnLuaDelegates::LoadLuaContext.IsBound())
	{
		FUnLuaDelegates::LoadLuaContext.Execute(ModuleName, CodeContext);
	}
