...
class FMenuBuilder;
class FScriptLogTextLayoutMarshaller;
class FTextLayout;
class SMenuAnchor;
...
struct FScriptLogMessage
{
	...
	FScriptLogMessage(const TSharedRef<FString>& NewMessage, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
	...
	FScriptLogMessage(const TSharedRef<FString>& NewMessage, ELogVerbosity::Type NewVerbosity, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
...
struct FScriptLogFilter
{
	...
	FScriptLogFilter() : TextFilterExpressionEvaluator(ETextFilterExpressionEvaluatorMode::BasicString)
	{
	...
	bool IsMessageAllowed(const TSharedPtr<FScriptLogMessage>& Message);
...
public:
	...
	SLATE_ARGUMENT(TArray< TSharedPtr<FScriptLogMessage> >, Messages)

		SLATE_END_ARGS()

		/** Destructor for output log, so we can unregister from notifications */
		virtual ~SScriptEditorLog();
	...
	/**
	 * Creates FHotPakerToolLogMessage objects from FOutputDevice log callback
	 *
	 * @param	V Message text
	 * @param Verbosity Message verbosity
	 * @param Category Message category
	 * @param OutMessages Array to receive created FHotPakerToolLogMessage messages
	 * @param Filters [Optional] Filters to apply to Messages
	 *
	 * @return true if any messages have been created, false otherwise
	 */
	static bool CreateLogMessages(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category, TArray< TSharedPtr<FScriptLogMessage> >& OutMessages);
...
protected:
	...
	TSharedPtr< FScriptLogTextLayoutMarshaller > MessagesTextMarshaller;
...
public:
	...
	FScriptLogFilter Filter;

	TSharedPtr<class SComboButton> ViewOptionsComboButton;
};

/** Output log text marshaller to convert an array of FHotPakerToolLogMessages into styled lines to be consumed by an FTextLayout */
class FScriptLogTextLayoutMarshaller : public FBaseTextLayoutMarshaller
{
public:

	static TSharedRef< FScriptLogTextLayoutMarshaller > Create(TArray< TSharedPtr<FScriptLogMessage> > InMessages, FScriptLogFilter* InFilter);

	virtual ~FScriptLogTextLayoutMarshaller();
...
protected:
	...
	FScriptLogTextLayoutMarshaller(TArray< TSharedPtr<FScriptLogMessage> > InMessages, FScriptLogFilter* InFilter);

	void AppendPendingMessagesToTextLayout();

	/** All log messages to show in the text box */
	TArray< TSharedPtr<FScriptLogMessage> > Messages;
